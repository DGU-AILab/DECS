...
assert_contains "$ENTRYPOINT" 'home_path_test()' "home path test helper"
assert_contains "$ENTRYPOINT" 'run_as_user test "$test_flag" "$path"' "Kerberos home metadata checks run as user"
assert_contains "$ENTRYPOINT" 'home_path_test -d "$USER_HOME"' "home existence check works through Kerberos user ccache"
assert_contains "$ENTRYPOINT" 'home_path_test -f "$USER_HOME/.bashrc"' "bashrc check works through Kerberos user ccache"
assert_contains "$ENTRYPOINT" 'home_path_test -f "$JUPYTER_CONFIG_FILE"' "Jupyter config check works through Kerberos user ccache"
assert_contains "$ENTRYPOINT" 'home_path_test -s "$vnc_password_file"' "VNC password reuse works through Kerberos user ccache"
...
assert_contains "$ENTRYPOINT" '# BEGIN DECS history export' "history export marker"
assert_contains "$ENTRYPOINT" 'mkdir -p "$HOME/.history"' "history export stays in user home"
assert_contains "$ENTRYPOINT" 'history -w "$HOME/.history/${current_time}.txt"' "history export writes without sudo"
...
assert_contains "$ENTRYPOINT" 'install_chrome_shortcut' "Chrome desktop shortcut installer"
assert_contains "$ENTRYPOINT" 'Chrome.desktop' "Chrome desktop launcher"
assert_contains "$ENTRYPOINT" 'Exec=decs-chrome' "Chrome launcher uses wrapper"
assert_contains "$ENTRYPOINT" 'service dbus start || true' "system D-Bus starts for GUI apps"
...
assert_not_contains "$ENTRYPOINT" 'sudo mv $current_time.txt /var/log/audit/' "history export does not sudo move from Kerberos NFS home"
